...
$sql = "SELECT subcategory.name as subcat_name,subcategory.status as subcat_status,category.id as cat_id FROM subcategory,category WHERE subcategory.id=:id AND subcategory.category_id=category.id";
...
<div class="right_col" role="main">
                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        ...
                        <div class="form-group">
                                ...
                                <input type="checkbox" name="status" <?php if($result['subcat_status']=='active'){echo 'checked';}?> class="js-switch" id="commercial" />
                                </div>
                            </div>    
                        <div class="form-group">
                                ...
                                <div class="col-md-5 col-sm-6 col-xs-12">
                                    <select name="category" required class="form-control" id="">
                                        ...
                                        while ($categories = $statement->fetch(PDO::FETCH_OBJ)){?>
                                            <option value='<?php echo $categories->id?>' <?php if($categories->id==$result['cat_id']){echo 'selected';}?> ><?php echo $categories->name?></option>
                                        <?php }
                                        ?>
                                    </select>
                                </div>
    ...
    if(isset($_POST['create-subcat'])){
        ...
        $sql = "UPDATE subcategory SET name=:name,category_id=:category,status=:status WHERE id=:id";
